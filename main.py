...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            login_user(user)
            return redirect(url_for('get_all_posts'))
    return render_template("login.html", loggedin=current_user.is_authenticated, form=form)
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, form=form, current_user=current_user, loggedin=True)
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", form=form, loggedin=current_user.is_authenticated, current_user=current_user)
